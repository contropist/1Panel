...
func (u *SnapshotService) SnapshotCreate(req dto.SnapshotCreate) error {
	...
	var snapStatus model.SnapshotStatus
	...
	wg.Add(8)
	itemHelper := snapHelper{Wg: &wg, FileOp: files.NewFileOp(), Ctx: context.Background()}
	...
	go snapJson(itemHelper, snapStatus.ID, jsonItem, backupPanelDir)
	...
	go func() {
		wg.Wait()
		global.Cron.Start()
		snapCompress(snapStatus.ID, rootDir)
